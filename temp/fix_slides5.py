        ...
        if has_chinese(inner) and '"' in inner:
            ...
            while k < len(inner):
                ...
                if c == '"':
                    ...
                    prev_is_chinese = has_chinese(prev_c)
                    next_is_chinese = has_chinese(next_c)

                    # Opening quote: next is Chinese, prev is NOT Chinese
                    if next_is_chinese and not prev_is_chinese:
                        fixed.append('《')  # 《
                    # Closing quote: prev is Chinese, next is NOT Chinese
                    elif prev_is_chinese and not next_is_chinese:
                        fixed.append('》')  # 》
                    else:
                        fixed.append(c)
                else:
