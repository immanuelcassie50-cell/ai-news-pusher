...
# Read existing sheet1.xml to get the basic structure
tree = ET.parse('D:/temp/xlsx_work/F3_work/xl/worksheets/sheet1.xml')
work_dir = 'D:/temp/xlsx_work/F3_work'
root2 = tree.getroot()

# Find and remove old sheetData
sd_elem = root2.find(f'{{{NS}}}sheetData')
...
sd.append(row_elem(2, [
    c('A2', t='s', s=1, v=STR['name']),
    c('B2', s=1),
    c('C2', t='s', s=1, v=STR['date']),
    c('D2', s=1),
    c('E2', t='s', s=1, v=STR['dept']),
    c('F2', s=1),
]))
...
# ROW 4: Column headers for section 1
sd.append(row_elem(4, [
    c('A4', t='s', s=4, v=STR['col_type']),
    c('B4', t='s', s=4, v=STR['col_status']),
    c('C4', t='s', s=4, v=STR['col_sat']),
    c('D4', t='s', s=4, v=STR['col_priority']),
    c('E4', t='s', s=4, v=STR['col_remark']),
]))

# ROWS 5-10: 6 reward types
types = [STR['t1'], STR['t2'], STR['t3'], STR['t4'], STR['t5'], STR['t6']]
for i, type_idx in enumerate(types):
    r = 5 + i
    sd.append(row_elem(r, [
        c(f'A{r}', t='s', s=1, v=type_idx),
        c(f'B{r}', s=1),
        c(f'C{r}', s=1),
...
sd.append(row_elem(11, [
    ...
    c('B11', s=2, f='SUM(B5:B10)'),
    c('C11', s=2, f='SUM(C5:C10)'),
]))
...
sd.append(row_elem(15, [
    c('A15', t='s', s=4, v=STR['col_dim']),
    c('B15', t='s', s=4, v=STR['col_freq']),
    c('C15', t='s', s=4, v=STR['col_sincere']),
    c('D15', t='s', s=4, v=STR['col_diverse']),
    c('E15', t='s', s=4, v=STR['col_visible']),
    c('F15', t='s', s=4, v=STR['col_subtotal']),
]))

# ROWS 16-18: recognition counts
...
sd.append(row_elem(21, [
    c('A21', t='s', s=4, v=STR['col_growth']),
    c('B21', t='s', s=4, v=STR['col_train']),
    c('C21', t='s', s=4, v=STR['col_mentor']),
    c('D21', t='s', s=4, v=STR['col_rotate']),
    c('E21', t='s', s=4, v=STR['col_promo']),
    c('F21', t='s', s=4, v=STR['col_subtotal']),
]))
...
# ROW 25: Section 4 header
sd.append(row_elem(25, [c('A25', t='s', s=4, v=STR['sec4'])]))

# ROW 26: Column headers
sd.append(row_elem(26, [
    c('A26', t='s', s=4, v=STR['col_exp']),
    c('B26', t='s', s=4, v=STR['col_rhythm']),
    c('C26', t='s', s=4, v=STR['col_collab']),
    c('D26', t='s', s=4, v=STR['col_transp']),
    c('E26', t='s', s=4, v=STR['col_resource']),
    c('F26', t='s', s=4, v=STR['col_subtotal']),
]))
...
# ROW 30: Section 5 header
sd.append(row_elem(30, [c('A30', t='s', s=4, v=STR['sec5'])]))

# ROW 31: Column headers
sd.append(row_elem(31, [
    c('A31', t='s', s=4, v=STR['col_ai_tool']),
    c('B31', t='s', s=4, v=STR['col_ai适用']),
    c('C31', t='s', s=4, v=STR['col_ai_effect']),
    c('D31', t='s', s=4, v=STR['col_ai_diff']),
    c('E31', t='s', s=4, v=STR['col_ai_prio']),
]))

# ROWS 32-36: AI scenarios
ai_items = [
...
for i, (name_idx, app_idx, eff_idx) in enumerate(ai_items):
    r = 32 + i
    sd.append(row_elem(r, [
        c(f'A{r}', t='s', s=1, v=name_idx),
        c(f'B{r}', t='s', s=1, v=app_idx),
        c(f'C{r}', t='s', s=1, v=eff_idx),
        c(f'D{r}', s=1),
        c(f'E{r}', s=1),
    ]))

# Add freeze pane to sheetView
sv = root2.find(f'{{{NS}}}sheetViews/{NS}sheetView')
# Remove old pane if exists
for p in sv.findall(f'{{{NS}}}pane'):
    sv.remove(p)
...
# Add column widths (before sheetData)
...
for min_c, max_c, width, cw in col_defs:
    col = ET.SubElement(cols, 'col')
    col.set('min', str(min_c))
    col.set('max', str(max_c))
    col.set('width', str(width))
    col.set('customWidth', str(cw))

# Insert cols before sheetData
root2.append(cols)
root2.append(sd)

tree.write('/tmp/xlsx_work/F3_work/xl/worksheets/sheet1.xml',
