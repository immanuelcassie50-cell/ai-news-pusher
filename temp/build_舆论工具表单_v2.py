...
# ============================================================
# STRING INVENTORY - All text content goes here
# ============================================================
strings = [
    # ===== Form 1: 舆论危机应对时间线管理表 =====
    # Sheet 1: 危机阶段判断表 (indices 0-49)
    ...
    "峰值期", "舆情达到传播顶点，讨论热度最高，舆论压力最大", "热搜前三，全网讨论量峰值，主流媒体跟进报道", "谨慎发声，必要时主动澄清，避免激化矛盾", "通常2-5天", "意见领袖表态；#话题#冲上热搜；媒体社论发布",
    ...
    # Sheet 5: 澄清对象优先级表 (indices 150-179)
    ...
    # Sheet 6: 关键关系维护表 (indices 180-209)
    ...
    # ===== Form 4: 发声时机判断工具 =====
    # Sheet 10: 三问判断表 (indices 310-349)
    ...
    "总分", "加权总分", "建议", "总分&lt;6分暂缓发声 | 6-10分谨慎发声 | &gt;10分考虑发声",

    # Sheet 11: 发声内容检查表 (indices 350-389)
    ...
    # Sheet 17: 运气成分分析表 (indices 540-569)
    ...
    # Sheet 20: 历史舆情记录 (indices 630-659)
    ...
    # Sheet 21: 应对效果评估 (indices 660-689)
...
# Rows 3-7: Phase data (爆发期, 发酵期, 峰值期, 衰减期, 平息期)
phases = [
    (8, 13, 18, 23, 28, 33, 38),   # 爆发期
    (9, 14, 19, 24, 29, 34, 39),   # 发酵期
    (10, 15, 20, 25, 30, 35, 40),  # 峰值期
    (11, 16, 21, 26, 31, 36, 41),  # 衰减期
    (12, 17, 22, 27, 32, 37, 42),  # 平息期
]
...
# Rows 9-11: Current status section
s1 += make_row(9, [make_cell("A9", 43, "s=\"4\""), make_cell("B9", 44, "s=\"4\""), make_cell("C9", 45, "s=\"4\"")])
s1 += make_row(10, [make_input_cell("A10", "s=\"1\""), make_input_cell("B10", "s=\"1\""), make_input_cell("C10", "s=\"1\"")])
s1 += make_row(11, [make_cell("A11", 46, "s=\"4\""), make_cell("B11", 47, "s=\"4\""), make_cell("C11", 48, "s=\"4\"")])

# Risk level dropdown row
s1 += make_row(12, [make_input_cell("A12", "s=\"1\""), make_input_cell("B12", "s=\"1\""), make_input_cell("C12", "s=\"1\"")])
...
for i in range(30):
    row = i + 3
    # C column has auto-trend formula
...
# ============================================================
sheet_configs = [
    # (sheet_num, title_idx, header_indices, data_rows)
    (4, 110, [110, 111, 112, 113, 114, 115, 116, 117], 20),  # 关系分类矩阵
    (5, 150, [150, 151, 152, 153, 154, 155, 156, 157], 20),  # 澄清对象优先级表
    (6, 180, [180, 181, 182, 183, 184, 185, 186, 187], 20),  # 关键关系维护表
    (7, 210, [210, 211, 212, 213, 214, 215, 216, 217], 20),  # 日常积累记录
    (8, 250, [250, 251, 252, 253, 254, 255], 15),            # 一致性检验表
    (9, 280, [280, 281, 282, 283, 284, 285], 15),            # 存量盘点仪表盘
    (10, 310, [310, 311, 312, 313, 314, 315, 316], 8),       # 三问判断表
    (11, 350, [350, 351, 352, 353, 354, 355], 20),           # 发声内容检查表
    (12, 390, [390, 391, 392, 393, 394, 395, 396, 397], 20), # 历史发声记录
    (13, 420, [420, 421, 422, 423, 424, 425, 426, 427], 15), # 核心团队联系表
    (14, 450, [450, 451, 452, 453, 454, 455, 456], 15),      # 预警信号清单
    (15, 480, [480, 481, 482, 483, 484, 485, 486, 487], 15), # 行动协议跟踪表
    (16, 510, [510, 511, 512, 513, 514, 515], 10),           # 真假复盘识别表
    (17, 540, [540, 541, 542, 543, 544, 545, 546], 10),      # 运气成分分析表
    (18, 570, [570, 571, 572, 573, 574, 575, 576, 577, 578], 20), # 改进跟踪表
    (19, 600, [600, 601, 602, 603, 604], 10),                # 个人信息总览
    (20, 630, [630, 631, 632, 633, 634, 635, 636, 637, 638], 20), # 历史舆情记录
    (21, 660, [660, 661, 662, 663, 664, 665, 666, 667, 668], 20), # 应对效果评估
]
