# -*- coding: utf-8 -*-
"""
德赛西威 - Word 修复版（6 份使用「·」分隔符）
word-batch-2 agent 反向覆盖了文件，主控重新生成
"""

import os, sys
sys.path.insert(0, r"D:\CC\temp")

# 复用 part2 的函数，但重写保存路径
exec(open(r"D:\CC\temp\gen_word_part2.py", encoding="utf-8").read().replace(
    'D-06-基础班-提示词模板评分卡（每人一份）.docx',
    'D-06-基础班·提示词模板评分卡（每人一份）.docx'
).replace(
    'D-07-基础班-AI工具地图评分表（每人一份）.docx',
    'D-07-基础班·AI工具地图评分表（每人一份）.docx'
).replace(
    'D-09-内训师班-AI教练技能评估表（每人一份）.docx',
    'D-09-内训师班·AI教练技能评估表（每人一份）.docx'
).replace(
    'D-10-内训师班-10项课程包交付物检查表（每人一份）.docx',
    'D-10-内训师班·10项课程包交付物检查表（每人一份）.docx'
))

# 强制重写保存函数
import gen_word_part2
...
def make_D09_fixed():
    ...
    add_table_with_header(doc, SCORING_HEADERS, rows_a, col_widths=[Cm(0.8), Cm(2.5), Cm(4.2), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.5)])
    ...
    add_table_with_header(doc, SCORING_HEADERS, rows_b, col_widths=[Cm(0.8), Cm(2.5), Cm(4.2), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.5)])
    ...
    add_table_with_header(doc, SCORING_HEADERS, rows_c, col_widths=[Cm(0.8), Cm(2.5), Cm(4.2), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.5)])
    ...
    add_table_with_header(doc, SCORING_HEADERS, rows_d, col_widths=[Cm(0.8), Cm(2.5), Cm(4.2), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.3), Cm(1.5)])
...
def make_D10_fixed():
    ...
    add_table_with_header(doc, headers, items, col_widths=[Cm(0.8), Cm(6.2), Cm(2), Cm(2), Cm(2), Cm(2), Cm(1.5)])
...
def make_D11_full():
    ...
    add_table_with_header(doc, headers, desensitize_rows, col_widths=[Cm(0.8), Cm(3), Cm(4.5), Cm(3.5), Cm(3.5)])
    ...
    add_table_with_header(doc, sensitive_headers, sensitive_rows, col_widths=[Cm(0.8), Cm(5), Cm(2.5), Cm(3.5), Cm(3.5)])
...
if __name__ == "__main__":
    print("=== 修复 word-batch-2 反向覆盖的 6 份 ===")
    make_D06_full()
    make_D07_fixed()
    make_D09_fixed()
    make_D10_fixed()
