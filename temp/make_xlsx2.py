#!/usr/bin/env python3
"""Build all 10 新员工 assessment xlsx files"""
import os, shutil, subprocess

def find_excel_skill():
    """Find Excel skill dir by checking for templates/minimal_xlsx subdirs"""
    skill_base = r"C:\Users\Administrator\.claude\skills"
    for entry in os.listdir(skill_base):
        full = os.path.join(skill_base, entry)
        tpl_path = os.path.join(full, "templates", "minimal_xlsx")
        if os.path.exists(tpl_path):
            return full
    return None

SKILL_RAW = find_excel_skill()  # bytes path
OUT   = r"D:\新课开发\测评表单"
TMP   = r"D:\temp"
...
def cp_template(n):
    d = os.path.join(TMP, f"xlsx_{n:02d}")
    src = os.path.join(SKILL_RAW, "templates", "minimal_xlsx")
    if os.path.exists(d):
        shutil.rmtree(d)
...
def do_pack(d, out):
    s = os.path.join(SKILL_RAW, "scripts", "xlsx_pack.py")
    # Unix paths for Git Bash python3
    dn = os.path.basename(d)
    du = f"/d/temp/{dn}"
    ou = f"/{out[0].lower()}/{out[3:].replace(chr(92),'/')}".replace("//","/")
    return sh(f'python3 "{s}" "{du}" "{ou}"')

def xml_esc(s):
    return s.replace("&","&amp;").replace("<","&lt;")

def build(num, title, questions, dims, interp):
    wd = cp_template(num)
    ...
    ss_lines = [f'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>',
                f'<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="{sc}" uniqueCount="{sc}">']
    for s in strings:
    ...
    # styles
    shutil.copy(os.path.join(SKILL_RAW,"templates","minimal_xlsx","xl","styles.xml"),
                os.path.join(wd,"xl","styles.xml"))
    ...
    out_path = os.path.join(OUT, "新员工10大测评", f"{num:02d}_{title}.xlsx")
    ok = do_pack(wd, out_path)
    print(f"  {'OK' if ok else 'FAIL'} {num:02d}_{title}")
    return ok
...
def make01():
    ...
    dims = ["技术/职能型","管理型","自主/独立型","安全/稳定型","创业创造型","服务奉献型","纯粹挑战型","生活方式型"]
    interp = [
        ("技术/职能型","您追求在专业领域成为专家，职业驱动力来自持续深化专业知识。"),
        ("管理型","您渴望整合资源、做决策、担责任，职业驱动力来自影响力和控制感。"),
        ("自主/独立型","您希望按自己的方式工作，职业驱动力来自工作自主权。"),
        ("安全/稳定型","您重视可预期性和长期承诺，职业驱动力来自稳定感。"),
        ("创业创造型","您渴望创造属于自己的东西，职业驱动力来自创造欲和成就感。"),
        ("服务奉献型","您以帮助他人为核心动力，职业驱动力来自对他人需求的满足。"),
        ("纯粹挑战型","您被解决难题和战胜挑战所驱动，职业驱动力来自竞争和征服。"),
        ("生活方式型","您追求工作与生活的平衡，职业驱动力来自生活质量。"),
    ]
    return build(1, "职业锚自测", questions, dims, interp)
...
def make02():
    ...
    interp = [
        ...
        ("群体社会化-低","建议主动参与团队活动，主动发起与同事的午餐或咖啡交流，主动了解团队中的非正式结构。"),
    ...
    return build(2, "新员工组织社会化适应量表", questions, dims, interp)
...
def make03():
    ...
    dims = ["行为局促","认知局促","情感局促","回避行为"]
    interp = [
        ("行为局促-高","您在行为层面表现出明显的职场局促感。建议通过刻意练习逐步降低敏感性。"),
        ("认知局促-高","您对他人评价的担忧程度较高，经常处于被评价的焦虑中。建议尝试认知重构。"),
        ("情感局促-高","您在情感层面感受到较强的职场不适感，建议从小范围、低强度的社交练习开始。"),
        ("回避行为-高","您表现出明显的回避倾向，可能会因为回避而错过重要的发展机会。"),
        ("综合分析","请关注得分最高的维度，这通常是您职场局促感的最主要来源。"),
    ]
    return build(3, "职场局促感诊断量表", questions, dims, interp)
...
def make04():
    ...
    dims = ["主动探索型","反思内化型","实践应用型","社交学习型","系统结构型"]
    interp = [
        ("主动探索型-主导","您是主动探索型的学习者，善于自我驱动、持续学习。"),
        ("反思内化型-主导","您是反思内化型的学习者，善于从经验中提炼智慧。"),
        ("实践应用型-主导","您是实践应用型的学习者，偏好从做中学。"),
        ("社交学习型-主导","您是社交学习型的学习者，偏好协作和讨论。"),
        ("系统结构型-主导","您是系统结构型的学习者，偏好有框架的学习方式。"),
        ("混合型","您的学习风格较为多元，没有单一主导类型。这种灵活性使您能适应不同的学习情境。"),
    ]
    return build(4, "职场学习行为风格测评", questions, dims, interp)

def make05():
    questions = [
        ["我真心喜欢这家公司的文化和价值观","我愿意把这份工作视为长期事业","我对公司有强烈的归属感","我会因为公司的成功而感到骄傲","我希望能够在这家公司长期发展","我关心公司的未来发展"],
        ["我留在公司是因为我觉得有道义责任","我应该对公司的培养心存感激","我有责任为公司的发展贡献力量","我尽量不在公司困难时离开","我觉得离职会对不起主管的信任","我认同忠诚是重要的职业品质"],
    ...
    dims = ["情感承诺","规范承诺","持续承诺"]
    interp = [
        ("情感承诺-高","您对公司有强烈的情感认同，对组织的依恋程度高。"),
        ("情感承诺-低","您对公司的情感联结较弱，可能将当前工作视为纯粹的利益交换。"),
        ("规范承诺-高","您有较强的职业责任感和忠诚度。"),
        ("持续承诺-高","您留在这家公司主要是因为已积累的资历、福利、人脉等使离职成本较高。"),
        ("高综合承诺","您在三个维度上都表现出较高的承诺水平，是高度投入的组织成员。"),
        ("低情感承诺+高持续承诺","您留在这家公司主要是因为不得不，而非真正的认同。需要注意职业倦怠风险。"),
    ]
    return build(5, "组织承诺感诊断量表", questions, dims, interp)
...
def make06():
    ...
    dims = ["工作内容","成长机会","上级关系","组织文化","薪酬福利"]
    interp = [
        ("工作内容-正落差","您对工作内容现实的评价高于预期，实际工作比您入职前的想象更具挑战性和吸引力。"),
        ("工作内容-负落差","您觉得实际工作比预期更缺乏挑战性或自主性，可能存在大材小用的感觉。"),
        ("成长机会-负落差","您感受到明显的成长落差，实际发展机会少于预期。这是新员工离职的主要驱动因素之一。"),
        ("上级关系-正落差","您与上级的关系体验超出预期，这是入职适应的良好基础。"),
        ("上级关系-负落差","您与上级的关系体验低于预期。建议主动与主管进行1on1，清晰表达您的期望和需求。"),
        ("综合-低落差","您在各维度的期望与现实较为匹配，入职体验整体满意度较高。"),
        ("综合-高落差","您在多个维度都感受到了期望落差。这种状态若持续，可能导致早期离职风险。"),
    ]
    return build(6, "新员工期望落差测评", questions, dims, interp)
...
def make07():
    ...
    interp = [
        ...
        ("任务角色清晰-低","您在任务角色认知上存在模糊感，可能导致工作方向偏差或责任推诿。"),
        ("关系角色清晰-高","您对关系角色的认知较为成熟，能较好地理解组织中的位置和人际结构。"),
        ("关系角色清晰-低","您在关系角色认知上较为迷茫，可能在组织政治和跨部门协作中感到困惑。"),
        ("双高-清晰型","您在任务和关系两个维度都有较高的角色清晰度，是快速适应的新员工。"),
        ("双低-迷茫型","您在两个维度都表现出较低的角色清晰度，这是入职适应困难的高风险信号。"),
    ]
    return build(7, "新员工角色清晰度测评", questions, dims, interp)
...
def make08():
    ...
    dims = ["内部网络建设","外部网络建设","网络维护","网络利用"]
    interp = [
        ("内部网络建设-高","您在组织内部的人际网络建设上表现积极，与跨部门同事建立了良好的关系。"),
        ("内部网络建设-低","您在组织内部的关系建设上较为被动，可能只与直接工作接触的同事有交流。"),
        ...
        ("综合-高","您在网络建设的各个环节都表现良好，拥有丰富且活跃的人际网络。"),
    ]
    return build(8, "职场人际网络建设行为量表", questions, dims, interp)
...
def make09():
    ...
    interp = [
        ...
        ("中度契合","您的价值观与公司文化有较好的匹配，但存在一些可能的摩擦点。"),
        ("低度契合","您在某些核心价值观上与公司文化存在明显差异。建议与直接主管或HR进行坦诚对话。"),
        ("严重不契合","您的核心价值观与公司文化存在较大冲突。强行适应可能导致长期的内心消耗和倦怠。"),
        ("契合维度分析-优势","在以下维度您的价值观与组织文化高度一致：这些是您在这家公司最舒适和最能发挥的地方。"),
        ("契合维度分析-风险","在以下维度您的价值观与组织文化存在落差：这些是需要您特别注意的领域。"),
    ]
    return build(9, "新员工价值观与企业文化契合度测评", questions, dims, interp)
...
def make10():
    ...
    dims = ["任务效能感","社交效能感","学习效能感","适应效能感"]
    interp = [
        ("任务效能感-高","您对完成工作任务有较强的信心。这种高任务效能感是高效工作的心理基础。"),
        ("任务效能感-低","您对任务完成缺乏信心，可能经常担心自己的工作表现。建议从力所能及的小任务开始建立信心。"),
        ("社交效能感-高","您在人际互动方面有较强的信心，能够较好地处理职场关系。"),
        ("社交效能感-低","您在人际互动方面缺乏信心，可能回避社交场合或在人际冲突中感到无力。"),
        ("学习效能感-高","您对学习新事物的能力充满信心。这种成长型思维使您在面对未知挑战时更加从容。"),
        ("学习效能感-低","您对学习新事物的信心不足，可能对新任务感到恐惧。"),
        ("适应效能感-高","您对适应新环境有信心。这种心理弹性使您能够更好地应对变化和不确定性。"),
        ("适应效能感-低","您对适应新环境缺乏信心，对变化和未知感到焦虑。"),
        ("综合-高自我效能","您在各个维度都表现出较高的自我效能感，这是职业发展的重要心理资本。"),
        ("综合-低自我效能","您在自我效能感上整体偏低，可能需要外部的支持和肯定来建立信心。"),
    ]
    return build(10, "新员工自我效能感诊断量表", questions, dims, interp)

if __name__ == "__main__":
    os.makedirs(os.path.join(OUT,"新员工10大测评"), exist_ok=True)
    results = {}
    for fn, num in [(make01,"01"),(make02,"02"),(make03,"03"),(make04,"04"),(make05,"05"),
                     (make06,"06"),(make07,"07"),(make08,"08"),(make09,"09"),(make10,"10")]:
        try:
            results[num] = fn()
