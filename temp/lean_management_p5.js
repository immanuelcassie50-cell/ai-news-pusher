...
function createScenarioSlide(pres, title, content) {
  let slide = pres.addSlide();
...
const learningMapData = [
  ['时段', '内容', '你会做什么', '你会带走什么'],
  ['上午', '方针管理三步曲', '理解战略→方针→行动的分解逻辑', '一份方针展开图'],
  ['上午', '日常管理三要素', '掌握课题设定、活动展开、结果确认', '日常管理运行方案'],
  ['下午', '方针监查', '学会检查进度、发现偏差、帮助解决', '方针监查记录模板'],
  ['下午', '持续改善文化', '理解Kaizen思维，建立改善提案机制', '改善提案制度设计']
];

slide2.addTable(learningMapData, {
...
slide2.addText(['时段', '内容', '你会做什么', '你会带走什么'].join('          '), {
  x: 0.5, y: 1.2, w: 9, h: 0.6,
  fontSize: 14, fontFace: 'Microsoft YaHei',
...
levels.forEach((level, i) => {
  ...
  if (i < levels.length - 1) {
    slide5.addText('↓', {
      x: 3.5, y: level.y + 0.55, w: 1, h: 0.15,
      fontSize: 14, color: COLORS.lightText, align: 'center'
    });
  }
});
...
// 表头
slide6.addShape(pres.shapes.RECTANGLE, {
  x: 5.2, y: 2.2, w: 4.3, h: 0.36,
  fill: { color: 'D45A5A' }
});
slide6.addText('部门                          负责内容', {
  x: 5.2, y: 2.2, w: 4.3, h: 0.36,
  fontSize: 11, fontFace: 'Microsoft YaHei',
  color: COLORS.white, bold: true, align: 'center', valign: 'middle'
});
...
const conditions = [
  { num: '01', title: '定性描述+定量指标', desc: '不能只有数字，要说清楚'做什么'' },
...
const confirmData = [
  ...
  ['指标明确', '用什么指标衡量进步'],
  ['目标透明', '目标数字是多少，现状是多少'],
  ['频率合适', '多久确认一次（每天/每周/每月）'],
  ['偏差分析', '如果落后于计划，分析原因'],
  ['对策响应', '偏差出现后，制定对策并跟踪']
];

slide14.addTable(confirmData, {
...
const freqData = [
  ...
  ['战略目标（年度）', '每月一次', '总经理/副总'],
  ['方针目标（季度）', '每周一次', '部门负责人'],
  ['日常目标（月/周）', '每日/每周', '班长/组长']
];

slide19.addTable(freqData, {
  x: 1.5, y: 1.4, w: 7, h: 2,
  colW: [2.5, 2.2, 2.3],
  border: { pt: 0.5, color: COLORS.mediumGray },
  fontFace: 'Microsoft YaHei',
...
slide19.addShape(pres.shapes.RECTANGLE, {
  x: 1.5, y: 1.4, w: 7, h: 0.5,
  fill: { color: COLORS.primary }
});

slide19.addText('目标类型                  检查频率                  监查人', {
  x: 1.5, y: 1.4, w: 7, h: 0.5,
  fontSize: 14, fontFace: 'Microsoft YaHei',
  color: COLORS.white, bold: true, align: 'center', valign: 'middle'
});
...
slide22.addTable(fiveLayerData, {
...
slide30.addText('层级              内容              时间节点          责任人          衡量指标          目标值', {
  x: 0.3, y: 1.2, w: 9.4, h: 0.45,
  fontSize: 11, fontFace: 'Microsoft YaHei',
  color: COLORS.white, bold: true, align: 'center', valign: 'middle'
});
