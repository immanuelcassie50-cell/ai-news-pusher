...
def P98():
    ...
    for r, row in enumerate(rows):
        ...
        for c, cell in enumerate(row):
            color = [RED, GREEN, GOLD, RED][r] if c == 0 else TXT
            b = c == 0
            sz = 10
...
def P99():
    ...
    for r, row in enumerate(rows):
        ...
        x = rx
        for c, cell in enumerate(row):
            color = [RED, GREEN, GOLD, RED, GREEN, GOLD][r] if c == 0 else TXT
            b = c == 0
            sz = 10
            rc(s, x, y, cw[c], rh, fill, line=LGT)
            tx(s, x + Inches(0.1), y, cw[c] - Inches(0.2), rh, cell, sz=sz, c=color, b=b, an=MSO_ANCHOR.MIDDLE)
...
def P100():
    ...
    for i, cov in enumerate(coverages):
        ...
        tx(s, x, y, Inches(5.7), Inches(0.4), "v “ + cov, sz=14, c=LGT, b=True)"
...
with open('gen_pptx_part2.py', 'a', encoding='utf-8') as f:
    f.write(code)
print('P98-P100 OK, lines:', sum(1 for _ in open('gen_pptx_part2.py', encoding='utf-8')))
