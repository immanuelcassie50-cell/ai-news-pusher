...
def fix_content(content):
    ...
    while i < len(content):
        ...
        # Find the end of the string: it's the last " before ", {" or ",\n
        # Look for ", {" or ", " or end of line after the string
        remaining = content[str_start+1:]

        # Find position of end quote - look for ", with possible whitespace
        # Strategy: find all " positions, the last one before ", " or ",)" is the end
        # Better: find the quote that's followed by ", " or ",)" or end of statement

        # Find all quote positions in remaining
        quote_positions = []
        p = 0
        ...
        # The end quote is likely the last one - but we need to check context
        # Look for pattern: quote, then comma, then space or )
        end_quote_pos = None
