...
// ============================================================
string contractsPath = Path.Combine(outputDir, "工具集锦01-05-契约模板.docx");
Console.WriteLine("Creating: " + contractsPath);

using (var doc = WordprocessingDocument.Create(contractsPath, WordprocessingDocumentType.Document)) {
...
Console.WriteLine("Created: " + contractsPath);

// ============================================================
// Create Scripts Document
// ============================================================
string scriptsPath = Path.Combine(outputDir, "工具集锦06-话术库.docx");
Console.WriteLine("Creating: " + scriptsPath);

using (var doc = WordprocessingDocument.Create(scriptsPath, WordprocessingDocumentType.Document)) {
    var mainPart = doc.AddMainDocumentPart();
    mainPart.Document = new Document(new Body());
    var body = mainPart.Document.Body!;
...
Console.WriteLine("Created: " + scriptsPath);

// ============================================================
// Create New Tools Document
// ============================================================
string newToolsPath = Path.Combine(outputDir, "工具集锦07-新增工具.docx");
Console.WriteLine("Creating: " + newToolsPath);

using (var doc = WordprocessingDocument.Create(newToolsPath, WordprocessingDocumentType.Document)) {
    var mainPart = doc.AddMainDocumentPart();
    mainPart.Document = new Document(new Body());
    var body = mainPart.Document.Body!;
    ...
    AddTableHeader(body, "序号", "期望领域", "我的隐性期望（未说出口的）", "如果没被满足，我会...", "我通常如何应对");
    string[] areas = { "反馈节奏", "沟通方式", "任务边界", "工作边界", "认可方式", "其他" };
    for (int i = 0; i < 6; i++) {
        AddTableRow(body, (i+1).ToString(), areas[i], "", "", "");
    }
    AddSeparator(body);
    ...
    string[] domains = { "领域一：反馈节奏", "领域二：沟通渠道", "领域三：任务交代", "领域四：响应时段", "领域五：认可方式" };
    foreach (var d in domains) {
        AddParagraph(body, d, true, "1F3864");
    ...
    AddTableHeader(body, "类别", "预警信号", "观察日期", "备注");
    string[,] signals = {
        { "沟通类", "对方回复变简短或延迟", "", "" },
    ...
    for (int i = 0; i < 10; i++) {
        AddTableRow(body, signals[i,0], signals[i,1], signals[i,2], signals[i,3]);
    }
    AddSeparator(body);
    ...
    AddTableHeader(body, "序号", "摩擦描述", "发生日期", "涉及成员", "处理方式", "状态", "下次跟进");
    for (int i = 0; i < 8; i++) {
        AddTableRow(body, (i+1).ToString(), "", "", "", "", "待处理/已解决", "");
    }
}
Console.WriteLine("Created: " + newToolsPath);

// ============================================================
// Create Cover Page
// ============================================================
string coverPath = Path.Combine(outputDir, "工具集封面.docx");
Console.WriteLine("Creating: " + coverPath);

using (var doc = WordprocessingDocument.Create(coverPath, WordprocessingDocumentType.Document)) {
    var mainPart = doc.AddMainDocumentPart();
    mainPart.Document = new Document(new Body());
    var body = mainPart.Document.Body!;
...
Console.WriteLine("Created: " + coverPath);
