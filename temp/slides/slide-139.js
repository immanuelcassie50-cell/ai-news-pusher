// slide-139.js - Module 4 Summary Slide
...
const slideConfig = { type: `content`, index: 139, title: `模块四：综合演练` };
function createSlide(pres, theme) {
  ...
  // Title
  slide.addText(`模块四：综合演练`, { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: `Microsoft YaHei`, color: theme.primary, bold: true });
  // 4 key points
  const points = [
    { num: `1`, title: `场景卡是核心工具`, desc: `贯穿整个课程，真实任务驱动学习` },
    { num: `2`, title: `判断力地图整合`, desc: `整合所有学习，形成完整框架` },
    { num: `3`, title: `迁移能力`, desc: `让你举一反三，应用到不同场景` },
    { num: `4`, title: `30天行动计划`, desc: `确保学习成果落地` }
  ];
  points.forEach((p, i) => {
    const y = 1.0 + i * 1.05;
    // Number circle
    ...
    slide.addText(p.title, { x: 1.4, y: y + 0.1, w: 3.5, h: 0.35, fontSize: 15, fontFace: "Microsoft YaHei", color: theme.primary, bold: true });
...
if (require.main === module) {
  ...
  pres.writeFile({ fileName: `D:/CC/temp/slides/slide-139-preview.pptx` }).then(() => console.log(`Created slide-139-preview.pptx`));
}