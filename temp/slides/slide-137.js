// slide-137.js - Module 2 Summary Slide
...
const slideConfig = { type: `content`, index: 137, title: `模块二：人必须做什么` };
function createSlide(pres, theme) {
  ...
  // Title
  slide.addText(`模块二：人必须做什么`, { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 28, fontFace: `Microsoft YaHei`, color: theme.primary, bold: true });
  // 4 key points
  const points = [
    { num: `1`, title: `问题定义`, desc: `不是任务是什么，而是真正要解决什么问题` },
    { num: `2`, title: `框架选择`, desc: `框架决定分析角度，选错框架分析就失去意义` },
    { num: `3`, title: `前提检验`, desc: `AI结论依赖前提，前提错了结论就错了` },
    { num: `4`, title: `价值判断`, desc: `多方案都有数据支撑时，靠价值观做取舍` }
  ];
  points.forEach((p, i) => {
    const y = 1.0 + i * 1.05;
    // Number circle
  ...
  slide.addText(`137`, { x: 9.3, y: 5.1, w: 0.4, h: 0.4, fontSize: 12, fontFace: `Arial`, color: `FFFFFF`, bold: true, align: `center`, valign: `middle` });
  return slide;
}
...
if (require.main === module) {
  ...
  pres.writeFile({ fileName: `D:/CC/temp/slides/slide-137-preview.pptx` }).then(() => console.log(`Created slide-137-preview.pptx`));
}