...
sectPr.AppendChild(new PageSize { Width = 16838, Height = 23840 });
...
Style CreateParagraphStyle(string id, string name, string outlineLevel, string fontSize, bool bold)
{
    ...
    pPr.AppendChild(new SpacingBetweenLines { Before = bold ? "360" : "200", After = "120" });
...
void AddP(string text, string styleId = "Normal", bool bold = false, string fs = "24")
{
    ...
    rPr.AppendChild(new RunFonts { Ascii = "微软雅黑", HighAnsi = "微软雅黑", EastAsia = "微软雅黑" });
    rPr.AppendChild(new FontSize { Val = fs });
    rPr.AppendChild(new FontSizeComplexScript { Val = fs });
    if (bold) { rPr.AppendChild(new Bold()); rPr.AppendChild(new BoldComplexScript()); }
    run.AppendChild(rPr);
...
Table CreateTable()
{
    ...
    tblPr.AppendChild(new TableWidth { Width = "100%", Type = TableWidthUnitValues.Pct });
...
void AddCh1(Body body)
{
    AddH1("第一章　课程导览");

    AddH2("1.1 课程介绍");

    AddP("本工作坊是一门以“做中学”为核心的设计型课程，学员在讲师的引导下，通过结构化萃取与AI辅助，将散落在不同人身上的岗位实战经验，沉淀为一套目标读者拿到手就能用、翻开就能照着做的实操手册。");

    AddH3("课程定位");
    AddP("不是“讲方法论让学员回去自己做”，而是“在工作坊里把手册做出来”。所有方法讲解都嵌入在做的过程中，学员是通过完成任务来掌握方法，而不是先学方法再去应用。");
    ...
    AddH2("1.2 三类人群画像与角色定位");
    AddP("本工作坊参与者分为三类人群，各有不同的角色定位和核心价值：");

    var tbl1 = CreateTable();
    AddTableRow(tbl1, new[] { "人群", "画像描述", "核心价值", "典型心态" }, true);
    AddTableCellData(tbl1, new[] { "经验较浅者", "入职1年以内或跨岗转入，对本岗位业务尚在摸索期", "提供读者视角——说出真实困惑与卡点，充当手册的目标读者代表", "我想知道遇到XX情况到底该怎么办" });
    AddTableCellData(tbl1, new[] { "业务骨干", "3年以上经验，绩效优秀，公认的会干活的人", "提供经验视角——贡献可复用的操作经验、判断逻辑、避坑技巧", "我知道怎么做，但让我写我真不知道从何下笔" });
    AddTableCellData(tbl1, new[] { "管理者", "负责团队管理或业务把控，了解制度要求和常见风险", "提供标准视角——把控内容的准确性、规范性与合规性", "沉淀经验是好事，但内容必须准确，不能有制度风险" });
...
void AddCh2(Body body)
{
    AddH1("第二章　行前准备");

    AddH2("2.1 需要提前准备的资料清单");
    AddP("请在课前收集并整理与本组主题相关的现有资料，作为工作坊素材输入：");
    ...
    AddH2("2.2 笔记本电脑和AI工具要求");
    AddP("请确保携带支持以下要求的笔记本电脑：");
    ...
    AddBullet("录音转文字工具：用于访谈记录转写");

    AddH2("2.3 录音设备准备");
    AddP("访谈环节需要全程录音，建议准备：");

    AddBullet("手机或录音笔（确保存储空间充足）");
    AddBullet("如有条件，可准备领夹式麦克风以提高录音质量");
    AddBullet("提前测试录音APP的可用性");

    AddH2("2.4 建议准备的一件工作经历");
    AddP("在课程开始前，请回忆并记录一个让你印象深刻的工作经历：");

    AddBullet("一个你曾经遇到困难的任务场景");
    AddBullet("你是如何一步步解决这个问题的");
    AddBullet("这个经历中有哪些如果当初有人告诉我就好了的经验");
    AddP("这个经历将在课程的经验萃取环节中使用，帮助你理解什么是好的手册内容。");

    AddPageBreak();
}

void AddCh3(Body body)
{
    AddH1("第三章　学习地图");

    AddH2("3.1 三天整体路线图");
    AddP("以下是三天工作坊的整体安排与核心产出：");

    // Day 1
    var tblD1 = CreateTable();
    AddTableRow(tblD1, new[] { "时段", "模块", "核心内容", "时长", "产出" }, true);
    AddTableCellData(tblD1, new[] { "上午", "模块一", "开场导入 + 阅读版手册设计逻辑", "90分钟", "理解手册定位与设计原则" });
    AddTableCellData(tblD1, new[] { "上午", "模块二", "课题定位表 + 三类人群结构化访谈", "90分钟", "课题定位表 + 访谈录音" });
    AddTableCellData(tblD1, new[] { "下午", "模块三前半", "AI生成手册设计方案 + 三级大纲搭建", "90分钟", "设计方案 + 内容大纲" });
    AddTableCellData(tblD1, new[] { "下午", "模块三后半", "手册主体内容填充与交叉验证", "90分钟", "内容初稿" });
    body.AppendChild(tblD1);

    AddP(" ");

    // Day 2
    var tblD2 = CreateTable();
    AddTableRow(tblD2, new[] { "时段", "模块", "核心内容", "时长", "产出" }, true);
    AddTableCellData(tblD2, new[] { "上午", "模块四前半", "工具包开发（流程图+话术模板+检查清单）", "90分钟", "核心工具包（经验证）" });
    AddTableCellData(tblD2, new[] { "上午", "模块四后半", "典型案例开发（正面+反面+情景选择）", "90分钟", "典型案例集" });
    AddTableCellData(tblD2, new[] { "下午", "工具与案例嵌入", "工具包与案例嵌入手册对应位置", "90分钟", "完整手册（含工具与案例）" });
    AddTableCellData(tblD2, new[] { "下午", "阶段复盘", "各组展示当前产出 + 讲师点评", "90分钟", "明确第三天优化重点" });
    body.AppendChild(tblD2);

    AddP(" ");

    // Day 3
    var tblD3 = CreateTable();
    AddTableRow(tblD3, new[] { "时段", "模块", "核心内容", "时长", "产出" }, true);
    AddTableCellData(tblD3, new[] { "上午", "模块五前半", "五步优化法（逐步讲解+逐步实操）", "90分钟", "完成五步优化后的手册" });
    AddTableCellData(tblD3, new[] { "上午", "模块五后半", "简版使用说明 + 手册整合编排", "90分钟", "简版使用说明 + 编排后手册" });
    AddTableCellData(tblD3, new[] { "下午", "组间交叉评审", "交叉评审 + 当场修改", "90分钟", "经互评迭代的高完成度手册" });
    AddTableCellData(tblD3, new[] { "下午", "成果展示与收尾", "各组展示 + 讲师点评 + 后续规划", "90分钟", "高完成度手册开发包全套" });
    body.AppendChild(tblD3);

    AddH2("3.2 每天的核心任务和产出");
...
void AddCh4(Body body)
{
    AddH1("第四章　模块学习指南");

    AddH2("4.1 第一天：手册设计逻辑 + 方向定位 + 经验萃取");

    AddH3("模块一：开场导入与手册设计逻辑");
    AddP("学习目标：", "Normal", true);
    AddBullet("区分阅读版实操手册与制度文件、培训课件的本质差异");
    AddBullet("说出好手册的四个设计原则（场景为入口、动作为核心、案例为骨架、工具为配套）");
    AddBullet("描述手册的三种使用场景及对应的内容组织逻辑");

    AddP("学习内容：", "Normal", true);
    ...
    AddH2("4.2 第二天：大纲搭建 + 内容填充 + 工具包 + 案例开发");

    AddH3("模块三：AI辅助生成手册框架与内容");
    AddP("学习目标：", "Normal", true);
    AddBullet("使用AI工具将访谈录音/素材转化为结构化的手册设计方案");
    AddBullet("搭建包含一级、二级、三级的完整内容大纲");
    AddBullet("按场景描述 + 操作步骤 + 注意事项完成手册主体内容填充");
    ...
    AddH2("4.3 第三天：五步优化 + 整合编排 + 互评 + 展示");

    AddH3("模块五：五步优化法与整合编排");
    AddP("学习目标：", "Normal", true);
...
void AddCh5(Body body)
{
    AddH1("第五章　配套资源索引");

    AddH2("5.1 工具表单清单");
    AddP("本课程提供以下工具表单，学员可在学习过程中使用：");

    var tblTools = CreateTable();
    ...
    AddH2("5.2 练习题库说明");
    AddP("本课程的练习设计以做中学为核心，每个模块都配套有针对性的实操练习：");
    ...
    AddH2("5.3 场景库使用");
    AddP("课程提供多种典型场景供学员参考和对标：");

    AddBullet("正面场景：好的手册案例，展示如何以场景为入口组织内容");
    AddBullet("反面场景：伪手册案例，分析制度汇编型、课件搬运型、经验堆砌型的问题");
    AddBullet("参考场景：往期优秀学员产出，展示可达成的质量标准");

    AddP("场景库的使用建议：", "Normal", true);
...
void AddCh6(Body body)
{
    AddH1("第六章　课后支持");

    AddH2("6.1 课后任务清单");
    AddP("工作坊结束后，建议按以下优先级完成剩余任务：");

    var tblTasks = CreateTable();
