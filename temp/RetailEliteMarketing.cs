...
var outputPath = "D:\\新课开发\\销售\\零售精英：培养关键技能，打造长效佳绩\\完整课程包\\11-对外宣传文案\\零售精英-对外宣传文案.docx";
...
// Color scheme: 红灰配色 (Red #C00000, Gray #595959, Dark #333333)
const string Red = "C00000";
const string DarkRed = "8B0000";
const string Gray = "595959";
const string DarkGray = "333333";
const string LightGray = "666666";
...
Paragraph MakeHeading(string text, int level)
{
    ...
    var rPr = new RunProperties();
    rPr.Append(new Bold());
    rPr.Append(new Color { Val = level == 1 ? Red : DarkGray });
...
Paragraph MakeBullet(string text, bool indent = false, string color = null)
{
    ...
    rPr.Append(new Color { Val = color ?? DarkGray });
...
Paragraph MakeHighlightBox(string text, string bgColor = "FCE4D6")
{
    ...
    var rPr = new RunProperties();
    rPr.Append(new FontSize { Val = "22" });
    rPr.Append(new Color { Val = DarkGray });
    rPr.Append(new RunFonts { Ascii = "Calibri", HighAnsi = "Calibri", EastAsia = "SimSun" });
    r.Append(rPr);
    r.Append(new Text(text) { Space = SpaceProcessingModeValues.Preserve });
    p.Append(r);
...
body.Append(MakePara("培养关键技能，打造长效佳绩", "Subtitle", color: DarkGray, fontSize: 32, center: true));
body.Append(MakePara(" ", "Normal", fontSize: 20)); // Spacer
...
body.Append(MakeBullet("价格谈判四步法 → 谈出双赢还保住利润", true);
...
body.Append(MakePara("培养关键技能，打造长效佳绩", "Normal", bold: true, color: DarkGray, fontSize: 28, center: true));
...
body.Append(MakePara("与其在旺季忙成一团，不如现在投资自己", "Normal", color: DarkGray, fontSize: 24, center: true));
