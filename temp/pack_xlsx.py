#!/usr/bin/env python3
"""Pack all 10 新员工 assessment xlsx from existing template dirs"""
import os, shutil, subprocess

OUT = r"D:\新课开发\测评表单"
TMP = r"D:\temp"

# Find xlsx_pack.py in skill directory (short-name approach)
...
print(f"Pack script: {repr(PACK_SCRIPT)}")
...
def do_pack(work_num, title):
    """Pack work dir to output xlsx"""
    wd = os.path.join(TMP, f"xlsx_{work_num:02d}")
    out = os.path.join(OUT, "新员工10大测评", f"{work_num:02d}_{title}.xlsx")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    # Use forward-slash Windows paths (MinGW python3 resolves D:/... correctly)
    wd_path = f"D:/temp/xlsx_{work_num:02d}"
    out_path = out.replace(os.sep, '/')
    ok, out_log, err_log = sh(f'python3 "{PACK_SCRIPT}" "{wd_path}" "{out_path}"')
    if not ok:
        print(f"    pack stderr: {err_log[:200]}")
    return ok
...
def build_ss_and_sheets(num, title, questions, dims, interp):
    """Write sharedStrings, sheet1-4 into work dir"""
    wd = os.path.join(TMP, f"xlsx_{num:02d}")
    n_q = sum(len(qs) for qs in questions)
    n_d = len(dims)
    base = 36 + n_d  # questions start here in sharedStrings
...
def get_data(n):
    d = {
        ...
        ], ["技术/职能型","管理型","自主/独立型","安全/稳定型","创业创造型","服务奉献型","纯粹挑战型","生活方式型"],
        [("技术/职能型","您追求在专业领域成为专家，职业驱动力来自持续深化专业知识。适合在技术导向型组织中发展。"),
         ("管理型","您渴望整合资源、做决策、担责任，职业驱动力来自影响力和控制感。适合走上管理岗位。"),
        ...
        [("情感承诺-高","您对公司有强烈的情感认同，对组织的依恋程度高。这种高情感承诺带来更高的组织公民行为和更低的离职意向。"),
         ...
         ("持续承诺-高","您留在这家公司主要是因为已积累的资历、福利、人脉等使离职成本较高。这是成本驱动的承诺，而非真正的认同。"),
        ...
        [("任务角色清晰-高","您对任务角色的认知非常清晰，能够准确理解自己的职责边界和上级期望。这是高效工作的重要前提。"),
        ...
        [("内部网络建设-高","您在组织内部的人际网络建设上表现积极，与跨部门同事建立了良好的关系。"),
         ...
         ("网络维护-高","您注重关系的维护，能够保持人际网络的活力。这使您的人在需要时能够发挥作用。"),
         ("网络利用-高","您善于利用人际网络获取信息和资源，能在需要时有效地调动关系。这是一种重要的职场政治技能。"),
        ...
        9: ("新员工价值观与企业文化契合度测评", [
            ...
            ["我认为这家公司重视工作的意义而非仅仅是薪酬","我认为这家公司的文化是开放透明的","我认为这家公司的员工普遍讲诚信","我认为这家公司给员工提供自主决策的空间","我认为这家公司提供稳定的工作环境","我认为这家公司的员工不断学习和成长","我认为这家公司支持工作与生活的平衡","我认为这家公司的员工愿意帮助他人","我认为这家公司鼓励创新和变革","我认为这家公司提供公平公正的工作环境"],
        ], ["个人价值观","组织文化认知"],
...
if __name__ == "__main__":
    ...
    for n in range(1, 11):
        ...
        try:
            title, questions, dims, interp = get_data(n)
            build_ss_and_sheets(n, title, questions, dims, interp)
            ok = do_pack(n, title)
            results[f"{n:02d}_{title}"] = ok
