...
# 基础目录
TEMPLATE_DIR = 「/tmp/ref_empty「  # 使用参考文件作为基础模板
...
def create_sheet_xml(title, tab_color, headers, rows, col_widths):
    ...
    max_row = len(rows) + 2  # 标题行 + 头部行 + 数据行
    ...
    for i, width in enumerate(col_widths):
        col_min = i + 1
        col_max = i + 1
        cols_xml += f'<col width=「{width}「 customWidth=「1「 min=「{col_min}「 max=「{col_max}「/>\n'

    # 构建sheetData
    sheet_data = 「「

    # 标题行
    sheet_data += f'''<row r=「1「 ht=「34「 customHeight=「1「>
...
'''
    ...
    for i, h in enumerate(headers):
        col = chr(ord('A') + i)
        header_cells += f'''<c r=「{col}3「 s=「2「 t=「inlineStr「>
        <is><t>{h}</t></is>
      </c>
'''
    sheet_data += f「<row r=\「3\「 ht=\「28\「 customHeight=\「1\「>\n{header_cells}</row>\n「
    ...
    for row_idx, row_data in enumerate(rows):
        ...
        for col_idx, cell_data in enumerate(row_data):
            ...
            row_cells += f'''<c r=「{col}{row_num}「 s=「{style}「 t=「inlineStr「>
        <is><t>{cell_data}</t></is>
      </c>
'''
        sheet_data += f「<row r=\「{row_num}\「 ht=\「22\「 customHeight=\「1\「>\n{row_cells}</row>\n「
...
def create_workbook_xml(sheet_names):
    ...
    for i, name in enumerate(sheet_names):
        ...
        sheets_xml += f'    <sheet xmlns:r=「http://schemas.openxmlformats.org/officeDocument/2006/relationships「 name=「{name_escaped}「 sheetId=「{i+1}「 state=「visible「 r:id=「rId{i+4}「/>\n'
...
'''
    for i in range(2, sheet_count + 1):
        rels += f'''  <Relationship Id=「rId{i+3}「
    Type=「http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet「
...
SHEET_F2 = {
    ...
    「rows「: [
        ...
        [「重大损害「, 「造成重伤、死亡或重大财产损失「, 「判断「重大」的一般标准「, 「「, 「「],
...
SHEET_F3 = {
    ...
    「rows「: [
        [「言语层面「, 「威胁、恐吓、挑衅性语言「, 「具体到「杀」「伤」等暴力词汇「, 「避免回应，保持距离「, 「「],
...
ALL_SHEETS_EMPTY = [SHEET_F1, SHEET_F2, SHEET_F3, SHEET_F4, SHEET_F5,
                    SHEET_F6, SHEET_F7, SHEET_F8, SHEET_F9, SHEET_F10]

# ============= 填好版示例内容 =============

def create_filled_examples():
    「「「为填好版创建示例数据「「「
    filled_sheets = []
    for sheet in ALL_SHEETS_EMPTY:
        filled_sheet = dict(sheet)
        filled_sheet[「rows「] = []
        for row in sheet[「rows「]:
            new_row = list(row)
            # 根据不同sheet添加示例内容
            if sheet[「name「] == 「F1_正当防卫构成要件自检「:
                if 「起因条件「 in row[0]:
                    new_row[3] = 「对方持棍破门而入「
                    new_row[4] = 「符合「
                elif 「时间条件「 in row[0]:
                    ...
                    new_row[3] = 「为阻止侵害取其性命「
                    new_row[4] = 「存疑「
            elif sheet[「name「] == 「F2_防卫限度判断「:
                if 「基本适应「 in row[0]:
                    new_row[3] = 「对方持棍，我方徒手「
                    new_row[4] = 「需综合判断「
                elif 「特殊正当防卫「 in row[0]:
                    new_row[3] = 「对方持刀行凶「
                    new_row[4] = 「可能适用「
                elif 「防卫过当「 in row[0]:
                    new_row[3] = 「存在较大争议「
                    new_row[4] = 「需法院认定「
            elif sheet[「name「] == 「F3_冲突升级预警「:
                if 「言语层面「 in row[0]:
                    new_row[4] = 「对方说「弄死你」「
                elif 「肢体层面「 in row[0]:
                    new_row[4] = 「对方挥拳冲来「
                ...
                if 「现场视频「 in row[0]:
                    ...
                    new_row[3] = 「已记录2名目击者「
                    new_row[4] = 「进行中「
            elif sheet[「name「] == 「F6_报警陈述记录「:
                if 「事件中「 in row[0] and len(new_row[0]) < 10:
                    new_row[3] = 「已核对「
            elif sheet[「name「] == 「F7_伤情鉴定跟踪「:
                if 「伤情初查「 in row[0]:
                    new_row[3] = 「2024-01-15「
                    new_row[4] = 「轻伤二级「
            elif sheet[「name「] == 「F8_案例分析记录「:
                if 「案件名称「 in row[0]:
                    new_row[1] = 「昆山龙哥正当防卫案「
                    new_row[2] = 「可参考「
                elif 「基本案情「 in row[0]:
                    new_row[1] = 「刘海龙持刀攻击于海明，刀脱手后于海明捡刀反击「
            elif sheet[「name「] == 「F9_行动计划表「:
                if 「提升风险意识「 in row[0]:
                    new_row[4] = 「进行中「
            elif sheet[「name「] == 「F10_资源速查「:
                if 「法律援助「 in row[0]:
                    new_row[2] = 「拨打12348「
                    new_row[4] = 「24小时服务「
...
GUIDE_SHEETS = [
    ...
    },
    {
        ...
        「rows「: [
            ...
            [「「要件解释「栏如何填写？「, 「填写法律对该要件的具体要求和解释，作为判断的标准依据。「, 「可参考《刑法》第20条及相关司法解释。「],
            [「判断结果填「符合「就安全了吗？「, 「不一定，自检只是初步判断，最终需要司法机关根据案件具体情况认定。「, 「建议及时咨询专业律师。「],
        ]
    },
    {
        「name「: 「F2使用说明「,
        ...
        「rows「: [
            ...
            [「如何判断「基本适应「？「, 「需要综合比较侵害行为的手段、强度、后果与防卫行为的手段、强度、后果。「, 「不是要求完全对等，而是要符合比例原则。「],
    ...
    },
    {
        ...
        「col_widths「: [22, 40, 35],
        「rows「: [
            [「为什么是「三秒「？「, 「紧急情况下没有时间详细分析，三秒是快速决策的参考时间框架。「, 「训练时反复练习，形成快速反应习惯。「],
    ...
    },
    {
        ...
        「rows「: [
            ...
            [「如何检验完成情况？「, 「设定明确的检验标准，如「能准确复述「「完成实操练习「等。「, 「定期回顾和调整计划。「],
        ]
    },
    {
        「name「: 「F10使用说明「,
...
def build_empty_form():
    「「「构建空表「「「
    print(「正在创建配套表单_空表.xlsx...「)
    work_dir = 「/tmp/xlsx_work_empty「

    # 复制模板
    copy_template(TEMPLATE_DIR, work_dir)

    # 更新workbook.xml
    sheet_names = [s[「name「] for s in ALL_SHEETS_EMPTY]
    ...
    # 删除旧的worksheets并创建新的
    worksheets_dir = f「{work_dir}/xl/worksheets「
    for f in os.listdir(worksheets_dir):
        os.remove(os.path.join(worksheets_dir, f))

    for i, sheet in enumerate(ALL_SHEETS_EMPTY):
        sheet_xml = create_sheet_xml(
            title=f「正当防卫边界 配套表单 — {sheet['name']}「,
            tab_color=sheet[「tab_color「],
            headers=sheet[「headers「],
            rows=sheet[「rows「],
            col_widths=sheet[「col_widths「]
        )
        with open(f「{worksheets_dir}/sheet{i+1}.xml「, 「w「, encoding=「utf-8「) as f:
            f.write(sheet_xml)
...
def build_filled_form():
    「「「构建填好版「「「
    print(「正在创建配套表单_填好版.xlsx...「)
    work_dir = 「/tmp/xlsx_work_filled「

    # 复制模板
    copy_template(TEMPLATE_DIR, work_dir)

    # 获取填好版内容
    filled_sheets = create_filled_examples()

    # 更新workbook.xml
    sheet_names = [s[「name「] for s in filled_sheets]
    workbook_xml = create_workbook_xml(sheet_names)
    with open(f「{work_dir}/xl/workbook.xml「, 「w「, encoding=「utf-8「) as f:
        f.write(workbook_xml)
    ...
    for i, sheet in enumerate(filled_sheets):
        sheet_xml = create_sheet_xml(
            title=f「正当防卫边界 配套表单（填好版）— {sheet['name']}「,
            tab_color=sheet[「tab_color「],
            headers=sheet[「headers「],
            rows=sheet[「rows「],
            col_widths=sheet[「col_widths「]
...
def build_guide():
    「「「构建使用指引「「「
    print(「正在创建表单使用指引.xlsx...「)
    work_dir = 「/tmp/xlsx_work_guide「

    # 复制模板
    copy_template(TEMPLATE_DIR, work_dir)

    # 更新workbook.xml
    sheet_names = [s[「name「] for s in GUIDE_SHEETS]
    workbook_xml = create_workbook_xml(sheet_names)
    with open(f「{work_dir}/xl/workbook.xml「, 「w「, encoding=「utf-8「) as f:
        f.write(workbook_xml)
    ...
    for i, sheet in enumerate(GUIDE_SHEETS):
        sheet_xml = create_sheet_xml(
            title=f「正当防卫边界 · 表单使用指引 — {sheet['name']}「,
            tab_color=sheet[「tab_color「],
            headers=sheet[「headers「],
            rows=sheet[「rows「],
            col_widths=sheet[「col_widths「]
