...
# First copy template
src_template = "C:/Users/Administrator/.claude/skills/Excel表格处理/templates/minimal_xlsx"
dst_work = "D:/CC/tmp_bsc_work"

if os.path.exists(dst_work):
    shutil.rmtree(dst_work)

shutil.copytree(src_template, dst_work)

# Create worksheets directory
os.makedirs(f"{dst_work}/xl/worksheets", exist_ok=True)

# Create sharedStrings.xml
shared_strings = '''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="68" uniqueCount="68">
  <si><t>战略执行与落地——平衡计分卡与OKR实战</t></si>
  <si><t>平衡计分卡战略地图</t></si>
  <si><t>编制日期</t></si>
  <si><t>编制部门</t></si>
  <si><t>战略主题</t></si>
  ...
  <si><t>贡献度</t></si>
  <si><t>填写说明</t></si>
  <si><t>1. 蓝色字体为需要填写的内容，黑色字体为自动计算结果</t></si>
  <si><t>2. 战略主题应与公司年度战略重点一致</t></si>
  <si><t>3. 每个维度建议填写3-5个战略目标</t></si>
  <si><t>4. 行动方案应具体可执行，包含时间节点</t></si>
  <si><t>5. 权重总和为100%，各维度权重可自行调整</t></si>
  <si><t>指标因果关系填写说明</t></si>
  <si><t>1. 从学习与成长→内部流程→客户→财务，形成价值创造链条</t></si>
  <si><t>3. 战略主题词</t></si>
  <si><t>战略目标编号规则：维度缩写+序号，如F1表示财务维度第1个目标</t></si>
  <si><t>指标编号规则：目标编号+.+序号，如F1.1表示财务维度第1个目标的第1个指标</t></si>
  <si><t>维度权重填写</t></si>
  <si><t>财务维度权重</t></si>
  <si><t>客户维度权重</t></si>
  <si><t>内部流程维度权重</t></si>
...
with open(f"{dst_work}/xl/sharedStrings.xml", 'w', encoding='utf-8') as f:
    f.write(shared_strings)
print("sharedStrings.xml created")

# Create workbook.xml
workbook_xml = '''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
    <sheet name="四维指标填写表" sheetId="1" r:id="rId1"/>
    <sheet name="指标间因果关系图" sheetId="2" r:id="rId4"/>
    <sheet name="战略目标分解表" sheetId="3" r:id="rId5"/>
  </sheets>
  <calcPr calcId="0"/>
</workbook>'''

with open(f"{dst_work}/xl/workbook.xml", 'w', encoding='utf-8') as f:
    f.write(workbook_xml)
print("workbook.xml created")
...
with open(f"{dst_work}/xl/_rels/workbook.xml.rels", 'w', encoding='utf-8') as f:
    f.write(workbook_rels)
print("workbook.xml.rels created")
...
with open(f"{dst_work}/[Content_Types].xml", 'w', encoding='utf-8') as f:
    f.write(content_types)
print("[Content_Types].xml created")
...
with open(f"{dst_work}/xl/worksheets/sheet1.xml", 'w', encoding='utf-8') as f:
...
with open(f"{dst_work}/xl/worksheets/sheet2.xml", 'w', encoding='utf-8') as f:
...
with open(f"{dst_work}/xl/worksheets/sheet3.xml", 'w', encoding='utf-8') as f:
    f.write(sheet3_content)
print("sheet3.xml created")

print("All BSC files created successfully!")
